use super::*;
use crate::mock::{
    new_test_ext, Event as TestEvent, Origin, System, Test,KittiesModule
};
...
fn create_success() {
    new_test_ext().execute_with(|| {
        ...
        let kitty_id = 1u32;
        // 创建Kitty
        assert_ok!(KittiesModule::create(Origin::signed(accound_id)));
        // 检查拥有者
        assert_eq!(KittyOwner::<Test>::get(kitty_id), Some(accound_id));
        // 检查创建成功事件
        assert_has_event!(Event::<Test>::KittyCreated(accound_id, kitty_id));
    });
}
...
fn create_failed_not_enough_balance() {
    new_test_ext().execute_with(|| {
        let accound_id: u64 = 30;
       
        assert_noop!(
			KittiesModule::create(Origin::signed(accound_id)),
